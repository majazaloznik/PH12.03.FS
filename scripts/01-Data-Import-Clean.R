...
policy <- cbind(eval(parse(text = paste0("df.", years[1]))), year = years[1])
...
codes <- select(codes, Country.code, region, sub.region, alpha.3)
...
full <- arrange(full, Country..name, year) %>%
  ...
  fill(Country.code, Policy.on.growth, Policy.on.fertility.level,
       Government.support.for.family.planning, region, sub.region, alpha.3 )
...
annual.growth.rate <- rename(annual.growth.rate, alpha.3 = Country.Code)
...
full <- read.table("data/complete.csv")
